...
public class IdentityLoadTask extends Task<List<Identity>> {
    ...
    protected List<Identity> call() throws Exception {
        try {
            ...
            String url = MainApp.getProperty(AppConstant.Props.BASE_URL, null) + AppConstant.UrlPath.IDENTITY_CALL;
            ResponseEntity<Identity[]> response = restTemplate.getForEntity(url, Identity[].class);
            if (response == null || response.getStatusCode() != HttpStatus.OK)
                return null;
            LOGGER.info("Identity fetched: {}", response.getBody().length);
