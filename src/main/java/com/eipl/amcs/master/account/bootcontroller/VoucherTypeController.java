...
public class VoucherTypeController {
    ...
    @GetMapping
    public ResponseEntity<List<VoucherType>> index() {
        try {
            List<VoucherType> list = service.findAll();
            if (list == null || list.isEmpty())
                return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);

            return new ResponseEntity<List<VoucherType>>(list, HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<String> nextCode(@RequestParam(name = "society", required = true) String societyCode) {
        try {
            LOGGER.info("Next Voucher Type no for Society: {}", societyCode);
            String code = nextCodeService.getNextCode("VoucherType", "code", societyCode, 0);
    ...
    @PostMapping
    public ResponseEntity<VoucherType> createVoucherType(@RequestHeader Map<String, String> headers, @RequestBody VoucherType dto) {
        return new ResponseEntity<>(service.save(dto, CommonUtil.getIdentityHeader(headers)), HttpStatus.CREATED);

    }

    @PutMapping
    public ResponseEntity<VoucherType> updateVoucherType(@RequestHeader Map<String, String> headers, @RequestBody VoucherType dto) {
        return new ResponseEntity<>(service.update(dto, CommonUtil.getIdentityHeader(headers)), HttpStatus.CREATED);

    }

    @DeleteMapping("/{code}")
    public ResponseEntity<?> deleteVoucherType(@RequestHeader Map<String, String> headers,
