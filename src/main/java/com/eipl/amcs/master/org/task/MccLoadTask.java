...
public class MccLoadTask extends Task<List<Mcc>> {
    ...
    protected List<Mcc> call() throws Exception {
        try {
            ...
            String url = MainApp.getProperty(AppConstant.Props.BASE_URL, null) + AppConstant.UrlPath.MCC;
            ResponseEntity<Mcc[]> response = restTemplate.getForEntity(url, Mcc[].class);
            if (response == null || response.getStatusCode() != HttpStatus.OK)
                return null;
            LOGGER.info("Mcc fetched: {}", response.getBody().length);
