...
public class HisaabMitraMilkCollectionDbSaveTask extends Task<Boolean> {
    ...
    protected Boolean call() throws Exception {
        try {
            ...
            DateTimeFormatter formatter3 = DateTimeFormatter.ofPattern("dd-MMM-yy", Locale.ENGLISH);
            DateTimeFormatter formatter4 = DateTimeFormatter.ofPattern("yyyy-MM-dd", Locale.ENGLISH);
            DateTimeFormatter formatter5 = DateTimeFormatter.ofPattern("yyyy/MM/dd", Locale.ENGLISH);
            ...
            try (Stream<String> lines = Files.lines(new File(filePath).toPath(), StandardCharsets.UTF_8)) {
                reader.lines().forEach(line -> {
                    if (lineno.get() == 0) {
                        ...
                        int codeEx = CommonUtils.strToInteger(arr[2]);
                        ...
                        try {
                            qty = new BigDecimal(arr[4]);
                            amt = new BigDecimal(arr[7]);
                            map.put("qty", qty);
                            map.put("amount", amt);
                        } catch (Exception e) {
                            map.put("qty", 1);
                        ...
                        try {
                            date = LocalDate.parse(arr[0], formatter1);
                        } catch (Exception e) {
                            try {
                                date = LocalDate.parse(arr[0], formatter2);
                            } catch (Exception e1) {
                                try {
                                    date = LocalDate.parse(arr[0], formatter3);
                                } catch (Exception e2) {
                                    try {
                                        date = LocalDate.parse(arr[0], formatter4);
                                    } catch (Exception e3) {
                                        try {
                                            date = LocalDate.parse(arr[0], formatter5);
                                        } catch (Exception e4) {
                                            e4.printStackTrace();
                                        }
                                    }
