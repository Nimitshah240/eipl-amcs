        ...
        try {
            ...
            try (Connection connection = DriverManager.getConnection(urlDb, "", AppConstant.PROMPT_DB_PASS)) {
                ...
                ResultSet resultSet = statement.executeQuery("select format(Dump_Date, 'mmm yyyy') as month, count(*) as count from dockside " +
                        "where  format(Dump_Date, 'yyyy-MM-dd') >= '" + fromDate.toString() + "' AND  format(Dump_Date, 'yyyy-MM-dd') <= '" +
                        toDate.toString() + "' group by format(Dump_Date, 'mmm yyyy')");
...
//                    resultSet = statement.executeQuery("select * from dockside where format(Dump_Date, 'mmm yyyy') = '" + month + "'");
                    ...
                    String shift = null;
                    while (resultSet.next()) {
                        if (resultSet.getString("memcode") == null ||
                                resultSet.getString("memcode").isEmpty() ||
                                resultSet.getString("memcode").equalsIgnoreCase("0000") ||
                                resultSet.getDouble("Weight") == 0)
                            continue;
                        Map<String, Object> map = new HashMap<>();
                        int codeEx = CommonUtils.strToInteger(resultSet.getString("memcode"));
                        map.put("memcode", MainApp.identityDto.getSociety().getCode() + String.format("%04d", codeEx));
                        ...
                        map.put("qty", new BigDecimal(resultSet.getString("Weight")));
                        ...
                        map.put("convqty", CommonUtils.convertQty(AppConstant.CollectionType.MEMBER_COLL, resultSet.getString("Weight")));
                        shift = resultSet.getString("Shift");
                        LocalDate date = resultSet.getDate("Dump_Date").toLocalDate();
                        ...
                        if (codeEx >= cowMin && codeEx <= cowMax)
                            ...
                            map.put("milktype", mapMilkType.get("C"));
                        map.put("sampleno", resultSet.getInt("Sample_Id"));
                        map.put("code", MainApp.identityDto.getDock().getDockNo() + "-" + ((LocalDateTime) map.get("collectiondate")).format(dateTimeFormatter)
                                + map.get("shift") + "-" + map.get("sampleno") + "-" + codeEx);
                    ...
                    try (Connection connMySql = DriverManager.getConnection(mysqlUrl, "root", AppConstant.EIPL_DB_PASS)) {
                        ...
                        Statement stForeignkey = connMySql.createStatement();
                        stForeignkey.execute("SET FOREIGN_KEY_CHECKS = 0;");
                        stForeignkey.close();
                        ...
                        for (List<Map<String, Object>> maps : listTemp) {
                            ...
                            for (Map<String, Object> map : maps) {
                                ...
                                pstmt.setString(21, map.get("memcode").toString());
                        ...
                        stForeignkey = connMySql.createStatement();
                        stForeignkey.execute("SET FOREIGN_KEY_CHECKS = 1;");
                        stForeignkey.close();
