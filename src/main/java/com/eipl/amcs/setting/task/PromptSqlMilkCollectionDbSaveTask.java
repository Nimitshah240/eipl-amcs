...
//                    resultSet = statement.executeQuery("select * from tblILedger");
                    ...
                    int i = 1;
                    while (resultSet.next()) {
                        if ((resultSet.getString("SabhasadId") == null || resultSet.getString("SabhasadId").isEmpty()) && resultSet.getDouble("Qty") > 0) {
                            Map<String, Object> local = new HashMap<>();
                            local.put("code", MainApp.identityDto.getSociety().getCode() + i);
                            i++;
                            local.put("amount", resultSet.getBigDecimal("Amount"));
                            local.put("qty", resultSet.getBigDecimal("Qty"));
                            local.put("rate", resultSet.getBigDecimal("Rate"));
                            mapLocalMilksale.add(local);
                            continue;
                        }
                        ...
                        if (codeEx >= cowMin && codeEx <= cowMax) map.put("milktype", mapMilkType.get("C"));
                    ...
                    try (Connection connMySql = DriverManager.getConnection(mysqlUrl, "root", AppConstant.EIPL_DB_PASS)) {
                        String sql = "INSERT INTO milk_collection(code,sample_no,collection_date,fat,snf,clr,water,density," + "lectose,protein,rtpl,qty,amount,is_weight_auto,is_quality_auto,is_avg_param,union_code," + "qty_mode,converted_qty,converted_qty_mode,member_code,shift_code,milk_type_code,milk_quality_type_code," + "society_code,dock_no,created_at,created_by) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        int current = 1;
...
//                    String mysqlUrl = "jdbc:mysql://localhost:3366/" + AppConstant.EIPL_DB_NAME;
                    try (Connection connMySql = DriverManager.getConnection(mysqlUrl, "root", AppConstant.EIPL_DB_PASS)) {
                        ...
                        connMySql.setAutoCommit(false);
                        PreparedStatement pstmt = connMySql.prepareStatement(sql);
                        for (Map<String, Object> map : mapLocalMilksale) {
                            pstmt.setString(1, map.get("code").toString());
                            pstmt.setBigDecimal(2, (BigDecimal) map.get("amount"));
                            pstmt.setString(3, MainApp.identityDto.getSociety().getCode() + "0001");
                            pstmt.setBigDecimal(4, (BigDecimal) map.get("qty"));
                            pstmt.setBigDecimal(5, (BigDecimal) map.get("rate"));
                            pstmt.setInt(6, 1);
                            pstmt.addBatch();
                        }
//                        pstmt.executeBatch();
