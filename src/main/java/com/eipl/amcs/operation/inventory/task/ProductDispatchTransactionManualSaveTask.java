...
public class ProductDispatchTransactionManualSaveTask extends Task<Object> {
    ...
    protected Object call() throws Exception {
        try {
            ...
            String url = MainApp.getProperty(AppConstant.Props.BASE_URL, null) + AppConstant.UrlPath.PRODUCT_DISPATCH + "/manualDispatchTransaction";

            ResponseEntity<ProductDispatchTransaction> response = this.update == 0 ?
                    restTemplate.exchange(url, HttpMethod.POST, new HttpEntity<>(dto), ProductDispatchTransaction.class) :
                    restTemplate.exchange(url, HttpMethod.PUT, new HttpEntity<>(dto), ProductDispatchTransaction.class);
