...
public class ProductStockLoadTask extends Task<List<ProductStock>> {
    ...
    protected List<ProductStock> call() throws Exception {
        try {
            ...
            String url = MainApp.getProperty(AppConstant.Props.BASE_URL, null) + AppConstant.UrlPath.PRODUCT_STOCK;
            ResponseEntity<ProductStock[]> response = restTemplate.getForEntity(url, ProductStock[].class);
            if (response == null || response.getStatusCode() != HttpStatus.OK)
                return null;
            LOGGER.info("ProductStock fetched: {}", response.getBody().length);
