...
public class SplitterSerial implements SerialPortDataListener {
...
//        if (endChar == c && readBuffer.toString().length() >= hardwareDevice.getLength()) {
        if (endChar == c && (hardwareDevice.getStartChar() == null || hardwareDevice.getStartChar().isEmpty() || readBuffer.toString().contains(hardwareDevice.getStartChar())) && readBuffer.toString().length() >= hardwareDevice.getLength() - 1) {
            ...
            LOGGER.info("SPLITTER SERIAL DATA BEFORE: {}", data);
            if (hardwareDevice.getDiscardChars() != null && !hardwareDevice.getDiscardChars().isEmpty()) {
                LOGGER.info("SPLITTER SERIAL DIS {}", hardwareDevice.getDiscardChars());
                data = data.replace("\n", "");
                data = data.replace("\r", "");
                data = data.replace("\\s", "");
            ...
            LOGGER.info("SPLITTER SERIAL DATA: {}", data);
            Map<String, String> list = ResponseParserUtilNew.parse(data, hardwareDevice.getRegEx());
            if (list != null && !list.isEmpty()) {
                if (callback != null)
                    callback.onResponseFromDevice(list, AppConstant.DEVICE_TAG.SPLITTER_TAG);
